...
-- Assume:
-- C = customers count
-- N = N orders per customer
-- K = Top-K
-- D = Depth of b-tree

-- Formula:

-- A ≈ C × N          有 N，沒 K     ← 成本由資料量決定
-- B ≈ C × (D + K)    有 K，沒 N     ← 成本由需求量決定

-- 資料量成長 → A 線性變慢，B 幾乎不動（只有 D 偶爾 +1）
-- 需求量成長（K 從 3 變 100）→ A 不動，B 線性變慢
-- K 大到接近 N 的時候，B 就沒有優勢了（你要的跟全部一樣多，何必精準定位）
